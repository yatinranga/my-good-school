...
public class TeacherServiceImpl extends BaseService implements TeacherService {
	...
	public TeacherResponse save(TeacherRequest request) {
		...
		// try {
		// emailFlag =
		// userService.sendLoginCredentialsBySMTP(userService.usernamePasswordSendContentBuilder(
	...
	public TeacherResponse update(TeacherRequest request, String teacherCid) {
		...
		if (!getUser().getRoles().stream().anyMatch(r -> r.getName().equalsIgnoreCase("Supervisor")
				|| r.getName().equalsIgnoreCase("Coordinator") || r.getName().equalsIgnoreCase("Head"))) {
	...
	public List<TeacherResponse> getAllTeachersOfSchool(String schoolCid) {
		// schoolCid = !getUser().getRoles().stream().anyMatch(r ->
		// r.getName().equalsIgnoreCase("MainAdmin") ||
		// r.getName().equalsIgnoreCase("Lfin")) ? getUser().getSchool().getCid() :
		// schoolCid ;
		// if(schoolCid == null)
		...
		if ((teachers = teacherRepository.findAllBySchoolCidAndGradesCidOrTeacherActivityGradesGradeCidInAndActiveTrue(
				schoolCid, gradeIds)) == null)
	...
	public List<TeacherResponse> getAllCoachesOfSchool(String schoolCid /* ,String gradeId */) {
		...
		// if(grades == null)
		// teachers=
		// teacherActivityGradeRepository.findAllTeacherByTeacherSchoolCidActiveTrue(schoolCid);
		// else
		List<Teacher> teachers = teacherActivityGradeRepository
				.findAllTeacherByTeacherSchoolCidAndGradeCidInAndActiveTrue(schoolCid, gradeIds);
		// findAllBySchoolCidAndIsCoachTrue();
		if (teachers == null)
			throw new ValidationException("No coaches found.");
		return teachers.stream().map(TeacherResponse::new).distinct().collect(Collectors.toList());
	...
	public List<TeacherResponse> findCoachesBySchoolCidAndActivityCid(String schoolCid, String activityCid) {
		...
		List<Teacher> teachers = teacherActivityGradeRepository
				.findAllTeacherByActivityCidAndTeacherSchoolCidAndGradeCidInAndActiveTrue(activityCid, schoolCid,
						gradeIds);
		if (teachers == null)
			throw new ValidationException(String.format(
	...
	public ResponseEntity<?> uploadTeachersFromExcel(MultipartFile file, String schoolCid) {
		...
		try {
			...
			for (int i = 0; i < teacherRecords.size(); i++) {
				...
				teacherResponseList.add(save(validateTeacherRequest(tempStudentsRecords, errors, schoolCid)));
	...
	// @Secured(AuthorityUtils.SCHOOL_STAKEHOLDER_CREATE)
	// @Override
	// public ResponseEntity<?> uploadManagementFromExcel(MultipartFile file, String
	// schoolCid) {
	//
	// if (file == null || file.isEmpty() || file.getSize() == 0)
	// throw new ValidationException("Pls upload valid excel file.");
	//
	// if(!getUser().getRoles().stream().anyMatch(r ->
	// r.getName().equalsIgnoreCase("MainAdmin") ||
	// r.getName().equalsIgnoreCase("Lfin"))) {
	// if(schoolCid == null ||
	// !schoolRepository.existsByCidAndActiveTrue(schoolCid))
	// throw new ValidationException(String.format("School with id : (%s) not
	// found.", schoolCid));
	// }
	// else
	// schoolCid = schoolRepository.findCidById(getUser().gettSchoolId());
	...
	// List<Map<String, Object>> managementRecords = new ArrayList<Map<String,
	// Object>>();
	//
	// try {
	...
	// List<Map<String, Object>> tempStudentsRecords = new ArrayList<Map<String,
	// Object>>();
	// tempStudentsRecords.add(managementRecords.get(i));
	// managementResponseList.add(save(validateTeacherRequest(tempStudentsRecords,
	// errors, schoolCid)));
	// }
	// } catch (IOException e) {
	//
	// throw new ValidationException("something wrong happened may be file not in
	// acceptable format.");
	// }
	// Map<String, Object> responseMap = new HashMap<String, Object>();
	...
	public TeacherResponse addOrRemoveActivitiesToTeachers(TeacherRequest request) {
		...
		// if (userId == null)
		// throw new ValidationException(
		// "No user logged in currently, kindly log in as School Management to assign
		...
		// "Not Authorized to assign activities to Teachers pls login as School or
		// Management Member.");
		if (request == null)
	...
	public List<ClubMembershipResponse> getMembershipDetails(String teacherCid, String schoolCid) {
		Long teacherId = null;
		List<String> gradeIds = null;
		List<StudentClub> membership;
		if (getUser().getRoles().stream().anyMatch(r -> r.getName().equalsIgnoreCase("Supervisor"))) {
			membership = studentClubRepository.findAllByTeacherIdAndActiveTrue(teacherId);
		} else {
			if (teacherCid == null)
			...
			membership = studentClubRepository.findAllByTeacherIdAndStudentGradeCidInAndActiveTrue(teacherId, gradeIds);
		}

		// if (teacherId == null)
		// throw new ValidationException("Login as teacher/coach to view pending
		// requests.");
		// membership =
		// studentClubRepository.findAllByTeacherIdAndActiveTrue(teacherId);
	...
	@Override
	@Secured(AuthorityUtils.SCHOOL_CLUB_MEMBERSHIP_FETCH)
	public List<ClubMembershipResponse> getMembershipDetailsbyClub(String clubId, String teacherCid, String schoolCid) {
		if (clubId == null)
		...
		// Long teacherId = null;
		// if(!getUser().getRoles().stream().anyMatch(r ->
		// r.getName().equalsIgnoreCase("Supervisor") ||
		// r.getName().equalsIgnoreCase("Coordinator") ||
		// r.getName().equalsIgnoreCase("Head"))) {
		// if(teacherCid == null)
		// throw new ValidationException("TeacherId cannot be null.");
		// teacherId = teacherRepository.findIdByCidAndActiveTrue(teacherCid);
		// }else {
		...
		List<StudentClub> membership;
		if (getUser().getRoles().stream().anyMatch(r -> r.getName().equalsIgnoreCase("Supervisor"))) {
			membership = studentClubRepository.findAllByTeacherIdAndActivityCidAndActiveTrue(
					teacherRepository.getIdByUserIdAndActiveTrue(getUserId()), clubId);
		} else {
			if (teacherCid == null)
				throw new ValidationException("TeacherId cannot be null.");
			teacherId = teacherRepository.findIdByCidAndActiveTrue(teacherCid);
			...
			membership = studentClubRepository
					.findAllByTeacherIdAndActivityCidAndStudentGradeCidInAndActiveTrue(teacherId, clubId, gradeIds);
		}
		...
		// requests.");
		// List<StudentClub> membership =
		// studentClubRepository.findAllByTeacherIdAndActivityCidAndActiveTrue(teacherId,
		// clubId);

		if (membership == null || membership.isEmpty())
			throw new NotFoundException("No membership application found for you.");
